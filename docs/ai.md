...
- **🔌 附属插件**：已加载的附属插件列表（名称 / 版本 / 命令数 / 描述 / 作者），附属插件通过 `registerAddon` 注册元数据后显示
- **📦 插件中心**：浏览/搜索 [HuHoBot 附属插件中心](https://addon.txssb.cn)，一键下载安装到 `addons/` 并热加载（同名目录可勾选强制覆盖）
- **⚙️ 配置**：分组表单编辑全部配置，保存后自动热重载生效；含 **扫码绑定**（生成二维码，手机 QQ 扫一次自动写入 AppID/Secret）
- 移动端自动折叠为汉堡菜单

控制台同样可用：`huhobot center [搜索词]` / `huhobot install <插件ID> [force]` / `huhobot qr`。
